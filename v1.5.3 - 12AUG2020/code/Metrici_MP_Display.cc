...
void readFromFile(std::fstream& configFile, std::string (&str)[15], std::string identity) {
...
int main(int argc, char *argv[]) {
  ...
  if(argc == 2) {
    ...
    if(argument.compare("-v") == 0) {
      system("echo 'Metrici_128x64_Display v1.5'");
    }
  } else {
    ...
    std::string current_dhcpcd_conf_content[15];
    std::string current_wpa_conf_content[15];
    ...
    while(true) {
      ...
      if(flock(config_lockFile, LOCK_SH) == 0) {
        ...
        if(configFile.is_open()){
          ...
          std::string new_display_settings[15];
        ...
        if(inputs[0] != url_response) {
          ...
          if(url_response.substr(0,6)=="#BEGIN") {
            ...
            for(int i=1; i<nr_of_lines-1; i++) {
              ...
              // new code: v1.5
              replace_str(lines[i], "@up", "↑");
              replace_str(lines[i], "@down", "↓");
              replace_str(lines[i], "@left", "←");
              replace_str(lines[i], "@right", "→");
              // end new code v1.5
      ...
      if(flock(wpa_lockFile, LOCK_SH) == 0) {
          ...
          if(wpa_txtFile.is_open()) {
            ...
            std::string new_wpa_txt_content[15];
            readFromFile(wpa_txtFile, new_wpa_txt_content, "new_wpa_txt");
            wpa_txtFile.close();

            int k = 0;
            while(new_wpa_txt_content[k].length()!=0) {
              k++;
            }
            std::cout<<"wpa line count= "<<k<<std::endl;
